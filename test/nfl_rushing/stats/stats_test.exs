defmodule NflRushing.StatsTest do
  ...
  describe "export_data/1" do
    ...
    test "with data returns list" do
      ...
      refute Stats.export_data() == []
      assert Stats.export_data() |> Enum.count() == 2
    end
    ...
    test "with whole name param given as filter must filter by name" do
      ...
      records = Stats.export_data(%{"name_filter" => player_one.name})
      ids = Enum.map(records, fn record -> record.id end)
    ...
    test "with partial name param given as filter must filter by name" do
      ...
      records = Stats.export_data(%{"name_filter" => "Testing"})
      ids = Enum.map(records, fn record -> record.id end)
      ...
      records = Stats.export_data(%{"name_filter" => "filter"})
      ids = Enum.map(records, fn record -> record.id end)
    ...
    test "order data by rushing_yards_total asc" do
      ...
      records =
        Stats.export_data(%{"order_field" => "rushing_yards_total", "order_direction" => "asc"})
    ...
    test "order data by rushing_yards_total desc" do
      ...
      records =
        Stats.export_data(%{"order_field" => "rushing_yards_total", "order_direction" => "desc"})
    ...
    test "order data by rushing_touchdowns_total asc" do
      ...
      records =
        Stats.export_data(%{
          "order_field" => "rushing_touchdowns_total",
          "order_direction" => "asc"
        })
    ...
    test "order data by rushing_longest_touchdown asc" do
      ...
      records =
        Stats.export_data(%{
          "order_field" => "rushing_longest_touchdown",
          "order_direction" => "asc"
        })
    ...
    test "with name param that does not match as filter must return empty list" do
      ...
      assert Stats.export_data(%{"name_filter" => "Not valid name"}) == []
    end
  end
end
